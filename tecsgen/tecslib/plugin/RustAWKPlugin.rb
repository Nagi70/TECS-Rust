...
#== MultiPlugin クラス
class RustAWKPlugin < MultiPlugin
    def self.get_plugin superClass
      # case when (つまりは ===) では、期待したように一致しない模様
      if superClass == SignaturePlugin then
        require_tecsgen_lib "tecslib/plugin/RustAWKSignaturePlugin.rb"
        return RustAWKSignaturePlugin
      elsif superClass == CelltypePlugin then
        require_tecsgen_lib "tecslib/plugin/RustAWKCelltypePlugin.rb"
        return RustAWKCelltypePlugin
      elsif superClass == CellPlugin then
        require_tecsgen_lib "tecslib/plugin/RustAWKCellPlugin.rb"
        return RustAWKCellPlugin
