...
EOT
    ...
    if $generating_region.get_n_cells != 0 then
      @signature_list.each{ |s|
        f.print "\t$(GEN_DIR)/../$(APPLNAME)/src/#{s.get_global_name.to_s.gsub(/(.)([A-Z])/, '\\1_\\2').downcase}.#{$rust_suffix} \\\n"
      }
    end
    ...
    gen_rust_celltype_names( f, "	$(GEN_DIR)/../$(APPLNAME)/src/", "_impl.#{$rust_suffix} \\\n", false, false )
...
#      next if b_inline_only == false && ct.is_all_entry_inline?
  ...
  def gen_rust_celltype_names( f, prepend, append, b_plugin, b_inline_only_or_proc = true )
    ...
    @celltype_list.each { |ct|
      ...
      f.print " #{prepend}#{ct.get_global_name.to_s.gsub(/(.)([A-Z])/, '\\1_\\2').downcase}#{append}"
    }
