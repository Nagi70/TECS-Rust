...
EOT
    ...
    if $generating_region.get_n_cells != 0 then
      ...
      rust_signature_str_list.uniq!
      rust_signature_str_list.each{ |s|
        f.print "	$(GEN_DIR)/../$(APPLNAME)/src/tecs_signature/#{s}.#{$rust_suffix} \\\n"
      }
    end
    ...
    gen_rust_impl_names( f, "	$(GEN_DIR)/../$(APPLNAME)/src/tecs_impl/", "_impl.#{$rust_suffix} \\\n", false, false )
    ...
    gen_rust_celltype_names( f, "	$(GEN_DIR)/../$(APPLNAME)/src/tecs_celltype/", ".#{$rust_suffix} \\\n", false, false )
