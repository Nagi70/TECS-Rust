...
#
# Rust files
#  RUST_PLUGIN_SIGNATURE      .rs files of signature
#  RUST_PLUGIN_CELLTYPE_SRCS  .rs files of Rust plugin generated celltype_impl.rs (celltype code)
#  RUST_PLUGIN_TECSGEN_SRCS   .rs files of Rust plugin generated celltype.rs
#  RUST_PLUGIN_SRCS           .rs files of Rust plugin generated
...
EOT
    ...
    # TODO: 以下の生成だと、Rustで生成されないシグニチャも対象となってしまうため、修正が必要
    f.print( "# RUST_PLUGIN_SIGNATURE: trait files generated by Rust plugin\n" )
    f.print( "RUST_PLUGIN_SIGNATURE = \\\n" )
    if $generating_region.get_n_cells != 0 then
      @signature_list.each{ |s|
        f.print "\t$(GEN_DIR)/../$(APPLNAME)/src/#{s.get_global_name.to_s.gsub(/([a-z0-9])([A-Z])/, '\1_\2').gsub(/([A-Z])([A-Z][a-z])/, '\1_\2').downcase}.#{$rust_suffix} \\\n"
      }
    end
    f.print "# RUST_PLUGIN_SIGNATURE terminator\n\n"

    f.print( "# RUST_PLUGIN_CELLTYPE_SRCS: sources of Rust celltype code written by user\n" )
    f.print( "RUST_PLUGIN_CELLTYPE_SRCS = \\\n" )
    gen_rust_impl_names( f, "	$(GEN_DIR)/../$(APPLNAME)/src/", "_impl.#{$rust_suffix} \\\n", false, false )
    f.print "# RUST_PLUGIN_CELLTYPE_SRCS terminator\n\n"

    f.print( "# RUST_PLUGIN_TECSGEN_SRCS: Rust celltype code generated by Rust plugin\n" )
    f.print( "RUST_PLUGIN_TECSGEN_SRCS = \\\n" )
    gen_rust_celltype_names( f, "	$(GEN_DIR)/../$(APPLNAME)/src/", ".#{$rust_suffix} \\\n", false, false )
    f.print "# RUST_PLUGIN_TECSGEN_SRCS terminator\n\n"
...
#      next if b_inline_only == false && ct.is_all_entry_inline?
  ...
  def gen_rust_celltype_names( f, prepend, append, b_plugin, b_inline_only_or_proc = true )
    dbgPrint "gen_rust_celltype_names #{@name}\n"

    @celltype_list.each { |ct|
      next if !(ct.get_n_cell_gen > 0 && ct.get_impl_lang == :Rust)
      f.print " #{prepend}#{ct.get_global_name.to_s.gsub(/([a-z0-9])([A-Z])/, '\1_\2').gsub(/([A-Z])([A-Z][a-z])/, '\1_\2').downcase}#{append}"
    }
  ...
  def gen_rust_impl_names( f, prepend, append, b_plugin, b_inline_only_or_proc = true )
    dbgPrint "gen_rust_impl_names #{@name}\n"
