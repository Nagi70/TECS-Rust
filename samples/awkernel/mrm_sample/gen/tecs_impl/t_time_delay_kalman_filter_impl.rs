...
impl STimeDelayKalmanFilter for EKalmanForTTimeDelayKalmanFilter<'_>{
	...
	fn update_with_delay(&'static self, y: &nalgebra::Matrix2x1<f64>, c: &nalgebra::Matrix2x6<f64>, r: &nalgebra::Matrix2<f64>, delay_step: &i32) -> Result<(), KalmanFilterError>{
		...
		// P_col = P[:, blk..blk+n] (dim_ex x 6); use on-the-fly access
		// Update x: x += P_col * z
		for row in 0..n_ex {
			let mut acc = 0.0;
			for k in 0..n { acc += lg.var.p[(row, blk + k)] * z[k]; }
			lg.var.x[row] += acc;
		}
		...
		// U = P_col * M (dim_ex x 6)
		let mut u: [[f64; 6]; 300] = [[0.0; 6]; 300];
		for row in 0..n_ex {
			for k in 0..n { // column in U
				let mut acc = 0.0;
				for j in 0..n { acc += lg.var.p[(row, blk + j)] * m[(j, k)]; }
				u[row][k] = acc;
			}
		}
		...
		for r in 0..n_ex {
			for cidx in 0..n_ex {
				let mut acc = 0.0;
				for k in 0..n { acc += u[r][k] * lg.var.p[(cidx, blk + k)]; }
				lg.var.p[(r, cidx)] -= acc;
			}
		}
