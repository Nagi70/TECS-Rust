/*
 * This file was automatically generated by tecsgen.
 * This file is not intended to be edited.
 */
#include "tTask_tecsgen.h"
#include "tTask_factory.h"


/* entry port descriptor referenced by call port (differ from actual definition) #_CPEPD_# */
extern struct tag_sTaskbody_VDES Taskbody1_eTaskbody_des;

extern struct tag_sTaskbody_VDES Taskbody2_eTaskbody_des;

extern struct tag_sTaskbody_VDES Taskbody3_eTaskbody_des;

extern struct tag_sTaskbody_VDES Taskbody4_eTaskbody_des;

/* call port array #_CPA_# */




/* array of attr/var #_AVAI_# */
/* cell INIB #_INIB_# */
tTask_INIB tTask_INIB_tab[] = {
    /* cell: tTask_CB_tab[0]:  Task1 id=1 */
    {
        /* call port (INIB) #_CP_# */ 
        &tTaskbody_INIB_tab[0],                  /* cTaskbody #_CCP2_# */
        /* attribute(RO) */ 
        1,                                       /* id */
    },
    /* cell: tTask_CB_tab[1]:  Task2 id=2 */
    {
        /* call port (INIB) #_CP_# */ 
        &tTaskbody_INIB_tab[1],                  /* cTaskbody #_CCP2_# */
        /* attribute(RO) */ 
        2,                                       /* id */
    },
    /* cell: tTask_CB_tab[2]:  Task3 id=3 */
    {
        /* call port (INIB) #_CP_# */ 
        &tTaskbody_INIB_tab[2],                  /* cTaskbody #_CCP2_# */
        /* attribute(RO) */ 
        3,                                       /* id */
    },
    /* cell: tTask_CB_tab[3]:  Task4 id=4 */
    {
        /* call port (INIB) #_CP_# */ 
        &tTaskbody_INIB_tab[3],                  /* cTaskbody #_CCP2_# */
        /* attribute(RO) */ 
        4,                                       /* id */
    },
};

/* entry port descriptor #_EPD_# */
